...
class TokenResponse(BaseModel):
    ...
    Now contains only the access token now as the refresh token is delivered via
    an HttpOnly cookie (Set-Cookie header), not in this response body.
    - access_token  ->  short-lived JWT (15 min),sent on every API request.

    Javascript cannot read HttpOnly cookies , which protects the refresh token from
    XSS attacks.
